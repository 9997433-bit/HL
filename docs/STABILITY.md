...
**Version:** `0.2.0` · **Status:** Stable release · **Date:** 2026-08-27

OpenFEMLab `0.2.x` releases target engineers who want a reproducible,
solver-independent modal correlation and model-updating workflow.  `0.2.0`
removes the beta tag: the core workflow is exercised end-to-end in CI and
industrial BDF ``RBE2`` ties are assembled into the solver.
...
| Industrial I/O | BDF subset (`GRID`, connectivity, `MAT1`, `PSHELL`/`PSOLID`/`PROD`, assembled `RBE2`, preserved `RBE3`), OP2 geometry/modes, UFF-55/58 |

...
**Version:** `0.2.2` · **Status:** Stable release · **Date:** 2026-08-28

OpenFEMLab `0.2.x` releases target engineers who want a reproducible,
solver-independent modal correlation and model-updating workflow.  `0.2.2`
adds TRI3/CTRIA3, BDF ``SPC1``/``CONM2``, and analytic truss geometric
``dK/da`` for shape optimization.
...
| Industrial I/O | BDF subset (`GRID`, connectivity, `MAT1`, `PSHELL`/`PSOLID`/`PROD`/`PBAR`, `CTRIA3`, `SPC1`, `CONM2`, assembled `RBE2`/`RBE3`), OP2 geometry/modes, UFF-55/58 |

...
注:早期版本曾在 FAST 档下测得 α=2 静默段增益 +4.93 dB —— 那是选档规则修正(守卫先行,3 MHz/20 mm/s → SLOW)之前的错误档位;当前 SLOW 档 Python 实测为 **+2.75 dB**。断言 Q1-2 阈值为 **>+1.5 dB**:MATLAB 移植按既定策略走**原生 MATLAB/Octave RNG**(统计断言,确定性链路由金标对比覆盖),10 seeds 中值随 RNG 流漂移 —— 实测 Python/numpy +2.75 dB、Octave 8.4 +2.67 dB、MATLAB R2022a +1.84 dB;1.5 dB 阈值在所有实测值下保留 >0.3 dB 裕量,仍断言真实的正合成增益(理想等权上限 10·log10(3)=4.8 dB)。

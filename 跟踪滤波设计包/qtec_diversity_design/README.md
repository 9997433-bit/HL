...
- α ∈ {1, 2, ∞}:α=1 为速度域 MRC(平稳噪声方差最优),α=∞ 纯选路,**α=2 为推荐默认**(对非高斯 click 尖峰的抑制显著强于 α=1,SNR 增益只差 ~0.2 dB)。

**实测(M=3,每路 CNR=6 dB,τ_c=50 µs,FAST 档,3 MHz burst,10 seeds 中值,对照为每 seed 每指标的最优单路 oracle)**:

| 指标 | 最优单路 (oracle) | 合成 α=1 | 合成 α=2 (默认) | 合成 α=∞ |
|------|------|------|------|------|
| 速度尖峰 (>0.4 m/s) | 38.5 | 23 | **10** | 9 |
| 静默段 SNR 增益 | 0 dB (基准) | +5.11 dB | **+4.93 dB** | +3.33 dB |
| 全暗 HOLD / 失锁时间 | 8.1% | 0.80% | **0.80%** | 0.80% |
| 幅值偏置 (R1 近无噪+散斑) | — | +1.0% | **+1.4%** | +1.5% |

...
function [ck, v4_sel, v4_pe] = V4_(v1, v2)
  ...
  % V5 (审计: v_peak=None 错档): v_peak 未知时按仪器最大速度 APP_V_PEAK_MAX
  % 保守评估守卫 (频段规则已废除) -- cfg_for_frequency(100e3) 必须选 FAST
  % 并上报 overrange.
  cfg_def = cfg_for_frequency(100e3);
  pe_def = tracking_error_rad(100e3, dp.APP_V_PEAK_MAX, ...
                              dp.BANDS.(cfg_def.band).fn);
  ok5 = strcmp(cfg_def.band, 'FAST') ...
        && strcmp(select_band(100e3), 'FAST') ...
        && strcmp(select_band(100e3, dp.APP_V_PEAK_MAX), 'FAST') ...
        && ~cfg_def.guard_ok && cfg_def.overrange ...
        && abs(cfg_def.phi_err - pe_def) < 1e-12;
  fprintf(['\n  v_peak 未知 (None) 的保守默认: 按 APP_V_PEAK_MAX=%.0f m/s ' ...
           '评估守卫 -> cfg_for_frequency(100e3): band=%s, ' ...
           'phi_err=%.2f rad, overrange=%s (频段规则已废除)\n'], ...
          dp.APP_V_PEAK_MAX, cfg_def.band, cfg_def.phi_err, ...
          v4b2s_(cfg_def.overrange));
  ck(end+1) = vt_check('V5', ['v_peak 未知默认 APP_V_PEAK_MAX=30 m/s ' ...
      '保守守卫: cfg_for_frequency(100e3) 选 FAST 且 overrange=True ' ...
      '(非频段规则)'], ok5, ...
      sprintf('band=%s, phi_err=%.2fr, guard_ok=%s, overrange=%s', ...
              cfg_def.band, cfg_def.phi_err, v4b2s_(cfg_def.guard_ok), ...
              v4b2s_(cfg_def.overrange)));

...
%     A8  noisy near-pi / phase-integrity statistics (N1, N2):
%         86 MHz physical front end + LPF, sudden-2pi-jump and net
%         fringe-error metrics (audit issues 1 & 2 follow-up)
...
%   raises an error (nonzero exit code) if any check fails.
  ...
  nz.a8_clean = [a8.np_clean, a8.sl_clean, a8.fr_clean];
  nz.a8_np_pct = a8.np_pct;
  nz.a8_sl_pct = a8.sl_pct;
  nz.a8_fr_pct = a8.fr_pct;
...
function k = kconst_()
  ...
  k.FE_NT = 1025;
  k.B_FE_A7 = 2 * dp.F_SIGNAL_MAX;   % physically consistent 86 MHz front end
  k.B_FE_A8 = 2 * dp.F_SIGNAL_MAX;   % same physical front end for A8 stats
  k.FADE_DB = -30.0;
  k.NSEED_FADE = 4;
  k.NSEED_STATS = 50;
  k.TINY = 1e-300;
...
function n = sudden_2pi_jumps_vs_true_(y, ph_true)
%Sudden 2pi jumps of unwrap(angle(y)) - ph_true between ADJACENT samples.
%   Renamed from slips_vs_true_ (audit issue 2): only catches ABRUPT jumps,
%   NOT a displacement-continuity proof -- see fringe_slip_vs_true_.
  ph = np_unwrap(angle(y(:)));
  n = sum(abs(diff(ph - ph_true(:))) > pi);
end

function n = fringe_slip_vs_true_(y, ph_true, lam)
%Net integer fringe error at the end of the record (audit issue 2).
%   Catches slow accumulating whole-cycle drift that the adjacent-sample
%   jump test cannot see.  homodyne: ph = 4*pi/lam * x, 1 fringe = lam/2
%   displacement = 2*pi of phase; the record ends in the post-burst quiet
%   window so filter group delay contributes ~0.
  ph = np_unwrap(angle(y(:)));
  dphi_end = ph(end) - ph_true(end);
  x_err = dphi_end * lam / (4 * pi);
  n = round(x_err / (lam / 2));   % == round(dphi_end / (2*pi))
end
...
function r = run_case_(cs)
  ...
  for s = 0:k.NSEED-1
    ...
    for ib = 1:3
      ...
      if strcmp(band, sel)
        row.(band).slips_noisy(end+1) = sudden_2pi_jumps_vs_true_(yf, sc.ph);
      end
    end
  end
...
function [e2e, ck] = A2_()
  dp = design_params();
  k = kconst_();
  vt_print_header(sprintf(['A2  端到端弱光仿真 (CNR=%.0fdB, 噪声带宽 ' ...
      'B_NOISE_ENBW=%.0fMHz, %d seeds, gear_filter/R1-R3 方法同 ' ...
      'validate_tracking)'], k.CNR_DB, dp.B_NOISE_ENBW / 1e6, k.NSEED));
...
function [a6, ck] = A6_()
  ...
  vt_print_header(sprintf(['A6  前端模型一致性 (审计项1): 30 m/s 时 fD_peak=' ...
      '%.1f MHz > B_NOISE_ENBW/2=%.0f MHz -- 参数化 B_frontend, ' ...
      '前端LPF作用于信号+噪声'], fD / 1e6, dp.B_NOISE_ENBW / 2e6));
  ...
  Bsig = 2 * dp.F_SIGNAL_MAX;   % 86 MHz: two-sided band passing +/-43 MHz
  Bs = [dp.B_NOISE_ENBW, dp.B_NOISE_ENBW, Bsig, 100e6, Bsig, 100e6];
  pws = [s2, s2, s2, s2, s2 * Bsig / dp.B_NOISE_ENBW, ...
         s2 * 100e6 / dp.B_NOISE_ENBW];
  ...
  ck(end+1) = vt_check('F2', sprintf(['真实 40 MHz 前端 (LPF 作用于信号) ' ...
      '无法通过 30 m/s: clean |err| > 20%% (前端须扩至 ≥ ±F_SIGNAL_MAX=' ...
      '%.0fMHz)'], dp.F_SIGNAL_MAX / 1e6), ...
...
function [a7, ck] = A7_()
  ...
  vt_print_header(sprintf(['A7  掉光重捕获 (审计项3): 30 m/s @ 100 kHz, ' ...
      '深衰落 %.0f dB, 时长 2/10/50 us, CNR 12/6/3 dB (B_frontend=%.0fMHz ' ...
  ...
  fprintf(['  rms_pre/post = 衰落前/重锁后+20us 的速度 RMS 误差; gap = 跨衰落相位' ...
           '滑移 (周). 衰落起点取速度峰值 (+38.7 MHz Doppler, 最坏).\n']);
  fprintf(['\n        时长   CNR |         invalid%%        relock us ' ...
  ...
  for idur = 1:3
    ...
    for icnr = 1:3
      ...
      fprintf('    %4.0fus %3ddB | %16s %16s %11.2f %12.2f %8.2f %11.0f\n', ...
  ...
  fprintf(['  2 us 衰落短于门控检测常数 (tauP=1us IIR + 0.25us 确认), invalid 标志' ...
           '覆盖率实测 %.0f-%.0f%% -- 不可靠;' ...
           ' 若产品需要标记亚微秒级衰落, 须另加快速幅度监测通道.\n'], ...
          100 * min(inv_2us), 100 * max(inv_2us));
  ck = struct('cid', {}, 'label', {}, 'ok', {}, 'detail', {});
  ck(end+1) = vt_check('D1', '全部 (时长×CNR×seed) 光恢复后重锁, relock ≤ 20 us', ...
      d1_ok, sprintf('最大 %.1f us (FAST acq_time=4us + 门控检测延迟)', d1_worst));
  ck(end+1) = vt_check('D2', '重锁后速度 RMS 误差恢复: 每组中值 post ≤ 1.5× pre', ...
      d2_ok, sprintf('最坏 post/pre = %.2f', d2_worst));
  ck(end+1) = vt_check('D3', ['衰落 ≥ 10 us: 全部 seed invalid 覆盖率 ≥ 60% ' ...
      '(HOLD/ACQUIRE => invalid 标志可用; 2 us 衰落仅报告不断言)'], d3_ok, ...
      sprintf('2us 覆盖率 %.0f-%.0f%% (短于检测常数)', ...
              100 * min(inv_2us), 100 * max(inv_2us)));
end
...
function [a8, ck] = A8_()
  dp = design_params();
  k = kconst_();
  vt_print_header(sprintf(['A8  案例c 含噪 near-pi / 相位完整性统计 (审计项4): ' ...
      '%d seeds, CNR=%.0fdB, FAST 档, gate=auto, B_frontend=%.0fMHz+前端LPF ' ...
      '(物理一致前端, 同 A6 v2/A7)'], k.NSEED_STATS, k.CNR_DB, k.B_FE_A8 / 1e6));
  fprintf(['  区分三种事件: near_pi = 鉴相器 |e|>2.8 rad 的噪声激励瞬时越界 ' ...
           '(代理量, 确定性峰值已达 1.5 rad);\n']);
  fprintf(['  sudden_2pi_jumps = unwrap(angle(y_full)) 相对真实相位的相邻样本 >π ' ...
           '突跳 (突发2π跳变, 原名"真滑周");\n']);
  fprintf(['  fringe_slip = 记录末端净整周条纹误差 round(Δφ_end/2π) ' ...
           '(慢累积整周漂移, 突跳检测不可见).\n']);
  fprintf(['  诚实声明: 突发2π跳变=0 只排除突跳, 不构成位移连续性证明 -- 慢漂移由 ' ...
           'fringe_slip 度量; clean 与含噪分开断言, 含噪下只要求有界.\n']);
  sc = make_scene_(100e3, k.V_MAX_APP);
  s2 = 10^(-k.CNR_DB / 10);
  zc = exp(1i * sc.ph) + ...
       complex_bandlimited_noise(sc.N, dp.FS, 20e6, 1e-10, np_rng_new(777));
  zc = fir_lp_same(zc, k.B_FE_A8 / 2, dp.FS, k.FE_NT);
  [yf, ~, ~, ~, dg] = vt_gear_filter(zc, 'FAST', 1e-10, 'always');
  np_clean = dg.near_pi_events;
  sl_clean = sudden_2pi_jumps_vs_true_(yf, sc.ph);
  fr_clean = fringe_slip_vs_true_(yf, sc.ph, dp.LAMBDA);
  nps = [];
  sls = [];
  frs = [];
  errs = [];
  for s = 0:k.NSEED_STATS-1
    ...
    z = exp(1i * sc.ph) + ...
        complex_bandlimited_noise(sc.N, dp.FS, k.B_FE_A8, s2, rh);
    z = fir_lp_same(z, k.B_FE_A8 / 2, dp.FS, k.FE_NT);
    [yf, ~, ~, ~, dg] = vt_gear_filter(z, 'FAST', s2, 'auto');
    nps(end+1) = dg.near_pi_events;
    sls(end+1) = sudden_2pi_jumps_vs_true_(yf, sc.ph);
    frs(end+1) = fringe_slip_vs_true_(yf, sc.ph, dp.LAMBDA);
    errs(end+1) = amp_err_pct_(vdisc_(yf), sc);
  end
  fprintf('\n  clean 参考: near_pi=%.0f, sudden_2pi_jumps=%.0f, fringe_slip=%.0f\n', ...
          np_clean, sl_clean, fr_clean);
  ...
  fprintf('    %s %7.0f %7.0f %7.0f %7.0f %7.0f\n', padr_('突发2π跳变 sudden_2pi', 22), ...
          pctile_(sls, 50), pctile_(sls, 90), pctile_(sls, 95), ...
          pctile_(sls, 99), max(sls));
  fprintf('    %s %7.0f %7.0f %7.0f %7.0f %7.0f\n', padr_('|净条纹误差| fringe', 22), ...
          pctile_(abs(frs), 50), pctile_(abs(frs), 90), pctile_(abs(frs), 95), ...
          pctile_(abs(frs), 99), max(abs(frs)));
  ...
  fprintf(['\n  文档化限值 (fallback 区 100 kHz/30 m/s, phi_err=1.5 rad, ' ...
           'CNR=3dB, B=86MHz+LPF): 突发2π跳变 p95 ≤ 3 / p99 ≤ 5 每 0.5 ms;\n']);
  fprintf(['  |净条纹误差| p95 ≤ 300 / p99 ≤ 400 (实测 10^1..10^2 周: 速度峰值处 ' ...
           '~10 us 级短暂失跟踪 (门控仍 LOCK, NCO 相位连续, 无相邻样本突跳)\n']);
  fprintf(['  累积整周漂移 -- 降级区 (overrange=True) 位移积分无效, 幅值/速度指标' ...
           '仍有效); near_pi 代理 p95 ≤ 700; 幅值误差中值 < 10%% (与 E3 一致),\n']);
  fprintf(['  p90 < 20%%. 案例b (10 kHz, 30 m/s) phi_err=0.151 rad 守卫内, 其含噪' ...
           '突跳见 E2 detail (noisy slips max).\n']);
  ck = struct('cid', {}, 'label', {}, 'ok', {}, 'detail', {});
  ck(end+1) = vt_check('N1', ['A8 clean 参考: near_pi=0, 突发2π跳变=0, ' ...
      '净条纹误差=0 (与 E3 clean 判据一致, 物理一致前端下重测)'], ...
      np_clean == 0 && sl_clean == 0 && fr_clean == 0, ...
      sprintf('near_pi=%.0f, sudden_2pi_jumps=%.0f, fringe_slip=%.0f', ...
              np_clean, sl_clean, fr_clean));
  ok = pctile_(sls, 95) <= 3 && pctile_(sls, 99) <= 5 ...
       && pctile_(abs(frs), 95) <= 300 && pctile_(abs(frs), 99) <= 400 ...
       && pctile_(nps, 95) <= 700 ...
       && pctile_(abs(errs), 50) < 10.0 ...
       && pctile_(abs(errs), 90) < 20.0;
  ck(end+1) = vt_check('N2', sprintf(['含噪 %d seeds: 突发2π跳变 p95≤3/p99≤5, ' ...
      '|净条纹误差| p95≤300/p99≤400 (位移积分在降级区无效, 见上), near_pi ' ...
      'p95≤700, |err| p50<10%%/p90<20%% (有界文档化限值, 非零缺陷)'], ...
      k.NSEED_STATS), ok, ...
      sprintf(['sudden_2pi p95=%.0f p99=%.0f max=%.0f, |fringe| p95=%.0f ' ...
               'p99=%.0f max=%.0f, near_pi p95=%.0f, ' ...
               '|err| p50=%.2f%% p90=%.2f%%'], pctile_(sls, 95), ...
              pctile_(sls, 99), max(sls), pctile_(abs(frs), 95), ...
              pctile_(abs(frs), 99), max(abs(frs)), pctile_(nps, 95), ...
              pctile_(abs(errs), 50), pctile_(abs(errs), 90)));
  a8 = struct('np_clean', np_clean, 'sl_clean', sl_clean, ...
      ...
      'sl_pct', [pctile_(sls, 50), pctile_(sls, 90), pctile_(sls, 95), ...
                 pctile_(sls, 99), max(sls)], ...
      'fr_pct', [pctile_(abs(frs), 50), pctile_(abs(frs), 90), ...
                 pctile_(abs(frs), 95), pctile_(abs(frs), 99), ...
                 max(abs(frs))], ...
...
function A4_(bounds, e2e)
  ...
  fprintf('    - 高速工况 SNR 增益从 SLOW 的 ~%+.0f dB 降到 FAST 的 ~%+.0f dB\n', ...
          g_slow, g_fast_100k);
  fprintf('      (@100 kHz): 物理必然 (环带宽换跟踪能力). 注意: 高速度不增加回光功率;\n');
  fprintf('      CNR 由表面回光决定, 30 m/s 与弱回光可同时存在 -- 本节高速结论均在\n');
  fprintf('      CNR=3 dB 弱光下实测 (fD_peak=%.1f MHz, 幅值误差中值 <5%%, A2),\n', ...
          fD30 / 1e6);
  fprintf('      更弱回光时 FAST 的 SNR 余量收窄 (建议 CNR >= 6 dB, 见设计方案 §2).\n');
  ...
  fprintf('    (1) 前端带宽 (A6 实测, 审计项1): 30 m/s 时 fD_peak=%.1f MHz 超过\n', ...
          fD30 / 1e6);
  fprintf('        B_NOISE_ENBW/2=%.0f MHz; 真实 40 MHz 前端会削掉信号\n', ...
          dp.B_NOISE_ENBW / 2e6);
  fprintf('        (A6 v1: clean 误差已 >20%%), 硬件前端必须通过\n');
  fprintf('        ±F_SIGNAL_MAX=±%.0f MHz (fs=250 MS/s 复采样支持), 且 CNR 指标\n', ...
          dp.F_SIGNAL_MAX / 1e6);
  ...
  fprintf('        SNR (见 study_fast_fn_options.py). 含噪突发2π跳变有界 (A8 实测\n');
  fprintf('        p95≤3 每 0.5 ms), 但净条纹漂移实测 10^1..10^2 周/0.5 ms (A8\n');
  fprintf('        fringe_slip) -- 降级区位移积分无效, 幅值/速度指标仍有效, 产品须按\n');
  fprintf('        overrange 上报. 若未来需求扩展到 f>100 kHz 且同时 30 m/s, 再评估\n');
  fprintf('        提高 FAST fn (滑周极限 %.0f kHz @30 m/s).\n', bounds.f_fastpi / 1e3);
  fprintf('    (3) 掉光行为 (A7 实测, 审计项3): 光恢复后 ~5 us 重锁, 速度精度恢复;\n');
  fprintf('        但跨衰落相位滑移 10^1..10^3 周 -- HOLD/ACQUIRE 期间必须置 invalid\n');
  fprintf('        标志, 任何衰落间隙上禁止位移积分; 短于 ~2 us 的深衰落不能保证被门控\n');
  fprintf('        标记 (检测常数限制).\n');
end

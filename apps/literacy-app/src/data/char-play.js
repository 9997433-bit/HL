 ...
 *   生成层   char-play-generated.js，由 scripts/gen-char-play.mjs 离线算好：
 *            全库 1820 字，每字一行「玩什么 + 一句话线索」。线索来自按需加载的
 *            课文包，玩法参考了字源语料——这两样运行时都够不着，所以先算好。
 *   模板补齐 连生成层都没有的字（绘本生字、搜索进来的生僻字），用字表里现成的
 *            信息（单元 / 部首 / 卡片图标）现挑一个模板、配齐道具。
 *
 * 后两层都标 templateFallback: true，`source` 分别是 generated / fallback。
 *
...
/**
 * gen-char-play.mjs 算好的那一层：汉字|模板|一句话线索。
 * 这里只做拆行，道具照样由上面的模板现算——索引不背道具，包体才留得住。
 */
const GENERATED = new Map()
for (const raw of PLAY_ROWS.split('\n')) {
  const line = raw.trim()
  if (!line) continue
  const [char, template = '', hint = ''] = line.split('|')
  if (char) GENERATED.set(char, { char, template, hint })
}

/** 生成层登记了几个字。字表长了而索引没重跑，check:data 会红。 */
export const TOTAL_GENERATED_PLAYS = GENERATED.size

/**
 * 生成器要问：不看富脚本、也不看生成层，这个字默认玩什么。
 * 拿它做输入才不会把上一次的生成结果又喂回去（生成必须幂等）。
 */
export function defaultPlayPlan(char) {
  const ctx = contextOf(char)
  return {
    char,
    theme: ctx.themeId,
    template: chooseTemplate(ctx),
...
/**
 * 富脚本用的是情境那一套玩法名（点点数、跟着划、涂颜色…），舞台只认得五种。
 * 名字对不上时按「最接近的机制」落到舞台的模板上，而不是丢掉脚本重新轮转——
 * 手写的旁白和道具是照着这个情境写的，机制越接近，文案越对得上。
 */
const RICH_TEMPLATE_ALIAS = {
  'count-tap': 'tap-reveal',
  'scene-poke': 'tap-reveal',
  'sort-buckets': 'tap-reveal',
  'pair-match': 'tap-reveal',
  'sound-tap': 'tap-reveal',
  'color-fill': 'tap-reveal',
  'pop-bubbles': 'rain-catch',
  'grow-tap': 'morph-story',
  'swipe-motion': 'morph-story',
  'trace-path': 'morph-story',
  'word-build': 'drag-parts'
}

/** 玩法按「富脚本 → 生成层 → 主题轮转」的顺序定，前面的舞台不认识就往后让。 */
function resolveTemplate(rich, gen, ctx) {
  const wanted = rich?.template ? (BUILDERS[rich.template] ? rich.template : RICH_TEMPLATE_ALIAS[rich.template]) : null
  for (const id of [wanted, gen?.template]) {
    if (id && BUILDERS[id]) return normalizeTemplate(id, ctx)
  }
  return chooseTemplate(ctx)
}

/* ---------------------------------------------------- 富脚本道具 → 舞台道具 */

/**
 * 富脚本写道具用的是自己那套小词汇（hero / items / stages / target / decoys /
 * pairs / goal），舞台吃的是模板道具（items / frames / cells / options / drops）。
 * 两边对不上时**不能**把生道具直接塞给舞台：孩子会拿到一张点不动的卡片。
 * 所以这里翻译一遍，翻不出来就退回模板道具——旁白、主角图标照旧是手写的那份，
 * 只是道具通用一点。宁可道具通用，也不能玩不成。
 */
const HAN = /\p{Script=Han}/u

/** 富脚本的一串图标（字符串或对象都收）→ 舞台要的 { id, emoji, label }。 */
function richItems(list, fallbackLabel) {
  return (Array.isArray(list) ? list : [])
    .map((raw, i) => {
      const value = typeof raw === 'string' ? { emoji: raw } : raw ?? {}
      const emoji = value.emoji ?? value.item ?? value.a ?? value.glyph ?? ''
      const label = value.label ?? value.name ?? value.bucket ?? value.caption ?? fallbackLabel
      return { id: `r${i}`, emoji, label }
    })
    .filter((item) => item.emoji && item.label)
}

function adaptTapReveal(rich, ctx) {
  const props = rich.props ?? {}
  const flatPairs = Array.isArray(props.pairs) ? props.pairs.flatMap((p) => [p.a, p.b]) : []
  const items = richItems(props.items ?? flatPairs, `和「${ctx.char}」有关的东西`)
  if (items.length < 2) return null
  if (props.hero && !items.some((i) => i.emoji === props.hero)) {
    items.unshift({ id: 'rh', emoji: props.hero, label: `「${ctx.char}」的样子` })
  }
  return { items, reveal: ctx.char, prompt: `点开 ${items.length} 个盖子` }
}

function adaptMorphStory(rich, ctx) {
  const stages = Array.isArray(rich.props?.stages) ? rich.props.stages : []
  if (stages.length < 2) return null
  const frames = stages.map((stage, i) => {
    const glyph = HAN.test(stage) ? stage : ''
    return {
      id: `f${i}`,
      ...(glyph ? { glyph } : { emoji: stage }),
      caption: i === 0 ? `一开始的样子` : i === stages.length - 1 ? `变好啦——「${ctx.char}」` : '慢慢在变'
    }
  })
  if (!frames[frames.length - 1].glyph) {
    frames.push({ id: `f${frames.length}`, glyph: ctx.char, caption: `变好啦——「${ctx.char}」` })
    frames[frames.length - 2].caption = '慢慢在变'
  }
  return { frames, target: ctx.char, button: '变！' }
}

function adaptEmojiHunt(rich, ctx, rand) {
  const props = rich.props ?? {}
  const target = props.target ?? props.hero
  const decoys = (Array.isArray(props.decoys) ? props.decoys : []).filter(Boolean)
  if (!target || decoys.length < 1) return null
  const need = Math.min(Math.max(Number(props.goal) || 1, 1), 4)
  const size = 12
  const cells = []
  for (let i = 0; i < size; i += 1) {
    const hit = i < need
    cells.push({
      id: `g${i}`,
      emoji: hit ? target : decoys[i % decoys.length],
      label: hit ? `「${ctx.char}」的图` : '不是这个',
      hit
    })
  }
  return { target, targetLabel: `「${ctx.char}」的图`, need, cells: shuffled(cells, rand) }
}

function adaptRainCatch(rich, ctx, rand, base) {
  const props = rich.props ?? {}
  const wanted = (Array.isArray(props.items) ? props.items : []).filter(Boolean)
  if (!wanted.length) return null
  const need = Math.min(Math.max(Number(props.goal) || wanted.length, 1), wanted.length)
  const misses = (base.drops ?? []).filter((d) => !d.hit).slice(0, 4)
  const drops = shuffled(
    [
      ...
      ...misses.map((d, i) => ({ ...d, id: `rm${i}` }))
    ],
    rand
  )
  const staticCells = drops.map((d, i) => ({ id: `rs${i}`, emoji: d.emoji, label: d.label, hit: d.hit }))
  return { target: wanted[0], targetLabel: `「${ctx.char}」的图`, need, drops, staticCells }
}

const RICH_ADAPTERS = {
  'tap-reveal': (rich, ctx) => adaptTapReveal(rich, ctx),
  'morph-story': (rich, ctx) => adaptMorphStory(rich, ctx),
  'emoji-hunt': (rich, ctx, rand) => adaptEmojiHunt(rich, ctx, rand),
  'rain-catch': (rich, ctx, rand, base) => adaptRainCatch(rich, ctx, rand, base),
  // 拼一拼考的是「这个字归哪个偏旁管」，答案只能来自字表部首，
  // 富脚本给的那串图标翻译不成选项，道具照模板来。
  'drag-parts': () => null
}

/** 舞台真的玩得起来才算数：翻译完照模板的最低要求验一遍，不合格就退回模板道具。 */
function fitsTemplate(templateId, props) {
  if (!props) return false
  if (templateId === 'tap-reveal') {
    const items = props.items
    return (
      Array.isArray(items) &&
      items.length >= 2 &&
      items.every((i) => i?.id && i?.emoji && i?.label) &&
      new Set(items.map((i) => i.id)).size === items.length
    )
  }
  if (templateId === 'morph-story') {
    const frames = props.frames
    return (
      Array.isArray(frames) &&
      frames.length >= 2 &&
      frames.every((f) => f?.caption) &&
      Boolean(frames[frames.length - 1]?.glyph)
    )
  }
  if (templateId === 'emoji-hunt') {
    const { cells, need, target } = props
    return Boolean(target) && Array.isArray(cells) && cells.length >= 4 && cells.filter((c) => c.hit).length >= need
  }
  if (templateId === 'drag-parts') {
    const { options, whole, answer } = props
    const right = Array.isArray(options) ? options.filter((o) => o.correct) : []
    return (
      Boolean(whole && answer) &&
      Array.isArray(options) &&
      options.length >= 2 &&
      right.length === 1 &&
      right[0].glyph === answer &&
      options.every((o) => o.glyph && o.name)
    )
  }
  if (templateId === 'rain-catch') {
    const { drops, need, staticCells } = props
    return (
      Array.isArray(drops) &&
      drops.filter((d) => d.hit).length >= need &&
      drops.every((d) => typeof d.x === 'number' && typeof d.duration === 'number') &&
      Array.isArray(staticCells) &&
      staticCells.filter((c) => c.hit).length >= need
    )
  }
  return false
}

function mergeRichProps(templateId, rich, ctx, base) {
  const adapted = RICH_ADAPTERS[templateId]?.(rich, ctx, rngOf(hashOf(ctx.char, 17)), base) ?? null
  const merged = adapted ? { ...base, ...adapted } : base
  return fitsTemplate(templateId, merged) ? merged : base
}

/**
 * 取一个字的「玩」场景。**永远不返回 null**：
 * 有富脚本用富脚本（缺的字段由模板补齐），没有就用生成层的玩法与线索句，
 * 生成层也没有（字表外的生字）就现算一关。
 *
 * @param {string} char 单个汉字；空值 / 多字时取第一个字符，实在没有就退到「字」
 * @returns {{char: string, theme: string, template: string, narration: string,
 *            props: object, templateFallback: boolean, source: string}}
 */
export function getCharPlay(char) {
  ...
  const gen = GENERATED.get(one)
  const templateId = resolveTemplate(rich, gen, ctx)
  const base = buildPlay(ctx, templateId)
  base.source = gen ? 'generated' : 'fallback'
  // 线索句是这个字自己的话，垫在模板那句通用旁白前面
  if (gen?.hint) base.narration = `${gen.hint}。${base.narration}`
  if (!rich) return base
  ...
  return {
    ...
    props: mergeRichProps(templateId, rich, ctx, base.props),

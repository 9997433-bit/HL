...
字表 200 字、16 个单元(`src/data/characters.js`),字表页一次只挂一个单元,
底部按单元翻页,避免两百张卡片同时进 DOM。

字表以 monorepo 共享基线 `shared/data/common-hanzi.json` 为准:基线里的字必须
全部出现在字表里、拼音一一对上,`check:data` 会守住这条,免得两处读音打架。
配套内容目前是 5 本三级分级绘本(`src/data/books.js`)和 20 条四字成语
(`src/data/idioms.js`,候选取自 `shared/data/idioms.json`)。
...
所以这条必须自动化。它还会校验字表规模(≥200 字)与共享基线的覆盖/拼音一致性、
字段完整性(声调/图标/组词拼音)、每个单元至少 5 个字、绘本 ≥5 本且覆盖 ≥3 个分级、
部首示例是否都在字表内、成语 ≥20 条及其四字拆解与情景题答案下标。

`gen:hanzi` 把必备字表(课程字 + 共享基线)和顺带收录的字(部首示例、成语、
绘本正文)分开处理:必备字缺笔顺数据会让构建直接失败,其余缺字只警告。
当前离线笔顺覆盖 334 个字,约 692 KB。

`smoke` 用无头 Chrome 依次打开 21 条路由,收集控制台报错与未捕获异常,
并检查:组件是否挂载、页面有没有漏出 `NaN`/`undefined`、
**详情页是否被重定向回列表**(内容改 id 之后最容易出现的回归)。
之后再跑 12 项交互:FSRS 到期队列、200 字字表的分页可达性、听音识字答题与换皮、
绘本翻到读完与逐句朗读、成语小剧场走到结尾、庆祝层跳过、

...
| R2-T09 | CI exit hardening | **Partial** — the `gates` job runs the import check, `ruff check .`, the registry consistency tests, the promotion gate and `-m acceptance`; 9 criteria are `verified` behind it. What remains is promoting the other 35 rows as their tracks close. |
...
| Core, elements & mesh | `core/`, `mesh/` | 18 + 42 + 61 + 66 + 76 + 17 (contracts) | Partial: 1D set, spatial beam, QUAD4, TET4, and HEX8 landed; AC-ELEM-001 is `verified`; the shell facet remains open. |
...
1. **Registry promotion.** All 44 criteria are covered (**34/34 P0,
   10/10 P1**): 9 are `verified` and the remaining 35 are `implemented`.
...
closed. The registry stands at **44/44 covered** (35 `implemented`,
9 `verified`), and the suite at **1,184 passed**.

...
| 项 | 结果 |
|---|---|
| `check:content` | 全部通过（新增步数对齐一节；合入 #7 后手写剖析 85 条覆盖 214/214） |
| `smoke.mjs` | 20 条路由 + 38 项交互，**0 项有问题**；其中「生活行星：母题规模与进阶档 —— 母题 214 类，进阶档题面标记正常」 |
| `vite build` | ✓ built in 7.72s，无告警 |
| `check:bundle` | 数学首屏 93936 B gzip < 250 KiB |
| `check:route-budget` | 18/18 组通过 |
| `check:round16` | 7/8，与基线同（唯一红项 H8 → `check:round15` 缺 android:sim 双 APK，与本改动无关）；`✓ H5 应用题剖析壳就位` 仍绿 |
| `check:round17` | 7/8，与基线同（唯一红项 H8 同上）；`✓ H4 精品剖析 50 ≥ 20` 仍绿 |
...
## 与 #7（手写剖析 50→85）的语义冲突，已在本分支修好

编排分支上 `db3fe12`（本分支的余数拆步）和 `eb65453`（#7 的剖析扩容）分开看都是绿的，
合到一起 `check:content` 报 **10 项不通过**：

```
✗ ceil-pack-shell  第 3 步退回了公式兜底（手写 2 句 / 算式 3 步）：35 ÷ 8 = 4 …… 3，4 + 1 = ?
✗ ceil-pack-space  第 3 步退回了公式兜底（手写 2 句 / 算式 3 步）
✗ ceil-pack-bamboo / ceil-pack-market / ceil-pack-snow  同上
✗ left-over-bakery 第 2 步退回了公式兜底（手写 1 句 / 算式 2 步）：27 ÷ 4 = 6 …… ?
✗ left-over-bug / left-over-post / left-over-reef / left-over-mine  同上
```

git 层面没有冲突，语义上有：#7 给这十张皮肤各写了一条**专属**剖析（皮肤专属 id
赢过语义 id），句数按老的步数写成 1 句 / 2 句；而有余数除法现在拆成两步，
各差一句，差的那一句退回公式兜底，整条链就算不上「老师讲法」。

本分支把 `r18-orchestration` 合回来并按拆步补齐了这十条：

| 剖析 | 句数 | 补的那一句 |
|---|---|---|
| `left-over-*` ×5 | 1 → **2** | 「装满的每一份都正好 b 个，把装进去的从 a 里拿走，剩下的就是答案」——只讲 a 和 b，不提商 |
| `ceil-pack-*` ×5 | 2 → **3** | 中间插「装满的部分每份 b 个、一共 quotient 份，从 a 里减掉」——这步不是问的那步，可以说商 |

合并后 `check:content` 全部通过（手写剖析 **85 条**覆盖 214/214），
`check-round18` 的 **H4 步数对齐 214/214 = 100.0%** 与 **H5 精品剖析 85 ≥ 80** 同时为绿。

**后续再写余数题剖析的人注意**：有余数除法现在是**两步**（`ceil-pack` 那种带进一法的是三步），
少写一句 `check:content` 就会报「第 N 步退回了公式兜底」。
`npm --prefix apps/math-app run report:wp-steps` 可以直接查每道母题该写几句。

## Round 18 门槛现状（合并后实测）

```
$ node scripts/check-round18.mjs
Round 18 check (ROUND18-v1.0): 4/8
...
H4 是本分支的责任项，已绿。其余四项归别的子代理，本分支不碰。

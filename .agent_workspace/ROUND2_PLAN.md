...
| R2-T02 3D elements | **PARTIAL** — QUAD4 (A37), TET4 (A46) and HEX8 (A59, merged by A79) are all on the integration branch with mesh generators and 203 tests, AC-ELEM-001..003 are registered as module M7 over all three (+24 acceptance cases) with AC-ELEM-001 **`verified`** through the R2-T09 gate (A72), and the spatial beam `BeamElement3D` (A82, 42 tests) was merged onto the integration branch at `75dd070` (A93). The `NeutralModel` → `Model` conversion shared with R2-T05 landed as `io/neutral_convert.py` (A106, 52 tests), so an imported rod/beam/quad/tet/hex block is now re-analyzable. Open: flat-facet shell, solid/shell BDF cards (`CBAR` included). |
...
- **Status: PARTIAL** — the three continuum slices and the spatial beam are **done**,
  ...
  **The AC-ELEM-* rows are now registered** (see the acceptance-links bullet below) and
  the neutral-model conversion landed with A106, so what remains open is the shell facet
  and the solid/shell BDF cards; the task does **not** close. See the R2-T02, A37, A46,
  A59, A82 and A106 entries in [`PROGRESS.md`](PROGRESS.md).
- **Scope:**
  ...
  - ~~`mesh/simple.py` generators for structured quad/hex blocks (needed for convergence
    fixtures) and neutral-model → assembly wiring for the new blocks~~ **landed**: the
    structured **quad**, **tet** (Kuhn-subdivided box) and **hex** generators, and
    `io/neutral_convert.py`, whose `neutral_to_model` turns an imported `ROD2`/`BEAM2`/
    `QUAD4`/`TET4`/`HEX8` block into bound elements (A106).
...
2. **R2-T02 — 3D elements** (GAP-02, P0): QUAD4/TET4/HEX8 plus the spatial beam so
   ...
   by A93); the shell facet and the solid/shell BDF cards remain. R2-T05 is unblocked
   for solid *and* frame meshes, and A106's `neutral_to_model` connected the two tracks:
   an imported block now reaches the assembler.

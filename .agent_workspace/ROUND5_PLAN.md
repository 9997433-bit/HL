...
| AC-PERF-004 / AC-IO-004 registry review | not applicable | No tests tagged with either ID exist in this branch or the linked agent worktrees. Registering testless rows would violate the registry consistency contract. |
| Verification | pending | Run focused checks, the benchmark gate, and the full pytest suite after committing the implementation. |

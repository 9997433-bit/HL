...
facts are worth recording because they decide what the element is exact at: `det J` of a
trilinear map is degree ≤ 2 per direction, so the **volume** and the **mass row sums**
(hence the total mass and the whole lumped matrix) are integrated exactly by the default
rule on *any* hexahedron, while the off-diagonal consistent-mass terms are only
quadrature-approximated on a distorted brick (0.09 % against `integration_order=3`, and
exact on any parallelepiped). Tests pin all three, including that the approximation is
...
(M7 = 3) and `VALID_MODULES`/`FAMILY_TO_MODULE`/`ID_REGEX` grow the family.
...
future formulation is covered by adding a row. Measured: patch defects 1.5e-16 / 2.8e-16
/ 2.8e-16, convergence ratios 4.005 / 4.095 / 4.005. QUAD4 and HEX8 return *identical*
axial numbers, which is the expected result rather than a coincidence — with `ν = 0` and
lateral motion suppressed both reduce to the same 1D axial discretization.

Verified from a private clone with `PYTHONPATH` pinned to it, Python 3.12.3 /
NumPy 2.5.2 / SciPy 1.18.1: full suite **930 passed**, `ruff check .` clean. The
baseline at the branch point `1acb89b` was **797** on the same clone; this slice
contributes **100** of the +133 (76 in `tests/test_hex8.py`, 24 in
`tests/acceptance/test_elements.py`), the rest being the AC-MODAL work described below.

**Working-tree hazard, and a new variant of it.** Two agents were writing `/workspace`
at once. This slice's `git add -A` swept a concurrent agent's in-flight
`solver/modal.py`/`exceptions.py` edits into commit `62eae5b`, and minutes later that
agent's `a3806d1` swept *this* slice's uncommitted AC-ELEM registration (both documents,
the registry rows and `tests/acceptance/test_elements.py`) into its own commit. Nothing
was lost or duplicated and the combined tree is green, but the two tasks are now
interleaved across three commits on this branch and cannot be separated by file. Then
the fallback failed too: the private clone this run made at `/tmp/.a59-priv-9f31` was
itself checked out and `reset --hard` by another agent within minutes, so *private
clones are only safe under an unguessable path*, and the only durable store is the
remote. The rule that follows from this run: in a shared checkout never stage with
`git add -A` — stage explicit paths — and push as soon as a slice is coherent.

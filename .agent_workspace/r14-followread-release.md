...
### 1.2 两条闸的负向实测（2026-08-28）

闸装上了得撞一次才算数。两次都是临时改文件、跑一遍、改回来：

| 伪造手段 | `check:round14` 的 H1 | `test:asr:evalset` |
|---|---|---|
| `device-rtf.json` 换成只填五个字段的注水 JSON | `deviceRtf=**true**`（那条腿被点亮） | **47/49**，红在「探针能放行的必定收货台也放行」+「真机没跑就得写在明处」 |
| 本文件末尾追加一个 GO 锚点词小节 | `release=**true**`（那条腿被点亮） | **48/49**，红在「放行文档与数据互锁」 |
| 不动（当前状态） | `available=false，recorded=0/300，release=false，deviceRtf=false，harness=true，smoke=true` | **49/49** |

第一列说明这两道闸**不在探针里**——`check-round14.mjs` 只读文件、不跑 harness，
所以伪造能骗过 H1 的那两条腿。它们拦在 G1（全量单测）那一关：
`npm --prefix apps/literacy-app test` 里包含 `test:asr:evalset`，harness 红就整条链红。
换句话说，H1 单看会被骗，**H1 + G1 一起看不会**。真要让 H1 独自扛住，
探针得改成调用收货台——那是验收口径的事，记在这里给下一轮定。
...
- **放行文档腿目前靠一条互锁保持红色。** `scripts/check-round14.mjs` 判「文档算不算 GO」时，
  ...
  连 §5.1 那张拒收码表里 `gate-inconsistent` 那一条都是为此改的名（原名里含着一个锚点词）。
  真到了可以放行的那天，两边同时松开（§6 第 5 步），顺序不会反。
- **两道新闸都不在探针里。** 见 §1.2：伪造能骗过 H1 的 release / deviceRtf 两条腿，
  拦住它的是 G1 的全量单测。要让 H1 独自扛住，得让 `check-round14.mjs` 去调收货台——
  这是验收口径的改动，留给下一轮定。

...
// Derivative of the Keys kernel w.r.t. its signed argument s.
inline double cubicKernelDeriv(double s) {
  const double a = -0.5;
  const double t = std::fabs(s);
  const double sign = (s < 0) ? -1.0 : 1.0;
  if (t <= 1.0) return (3.0 * (a + 2.0) * t - 2.0 * (a + 3.0)) * t * sign;
  if (t < 2.0) return (3.0 * a * t * t - 10.0 * a * t + 8.0 * a) * sign;
  return 0.0;
}
...
void bicubicWithGrad(const Image& img, double x, double y, double& value,
  ...
  for (int m = -1; m <= 2; ++m) {
    wx[m + 1] = cubicKernel(fx - m);
    wy[m + 1] = cubicKernel(fy - m);
    dwx[m + 1] = cubicKernelDeriv(fx - m);
    dwy[m + 1] = cubicKernelDeriv(fy - m);
  }
